<script lang="ts">
  import { contributionGraphData } from "$lib/contributionGraphData";
  import Project from "$lib/Project.svelte";
  import {webProjects} from "./projects";

  const maxContributions = contributionGraphData.data.user.contributionsCollection.contributionCalendar.weeks.reduce((acc, week) => {
...
<main class="text-lg p-4">
  <div class="text-slate-50 max-w-6xl m-auto space-y-10">
    <div class="flex justify-between w-full">
      {#each contributionGraphData.data.user.contributionsCollection.contributionCalendar.weeks as item}
    ...
    <h1 class="text-5xl font-bold border-b-[5px] pb-2 border-dotted">Web development</h1>
